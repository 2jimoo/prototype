...
class EncoderModel(nn.Module):
    ...
    def forward(self, query: Dict[str, Tensor] = None, passage: Dict[str, Tensor] = None, identity: Tensor = None, oldemb: Tensor = None):
        ...
        if self.training:
            ...
            if identity is not None:
                target_fullupdate = self.compute_similarity(q_reps, p_reps).view(q_reps.size(0), -1)
                p_reps[identity] = oldemb.clone()
            ...
            if identity is not None:
                compatible_loss = self.compute_compatiblelogit_loss(scores, target_fullupdate)
